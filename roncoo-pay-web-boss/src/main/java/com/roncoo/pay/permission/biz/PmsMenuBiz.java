...
public class PmsMenuBiz {
    ...
    private void recursionTreeMenu(String pId, StringBuffer buffer, List list, String url) {
        ...
        for (Map map : listMap) {
            ...
            buffer.append("<li><a onclick=\"onClickMenuNode(").append(id).append(")\"  href=\"").append(url).append("?id=").append(id).append("\" target=\"ajax\" rel=\"jbsxBox\"  value=").append(
					id).append(">").append(name).append("</a>");
            if (!isLeaf.equals("1")) {
